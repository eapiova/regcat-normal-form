...
-- ────────────────────────────────────────────────────────────────────
-- Phase F.5 (post-Tier-2): composeCompFits family extracted.
-- Each takes 1-2 SCC2 callbacks (the recursive call function plus, where
-- applicable, fitsToCompFits / fitsEqToCompFitsEq for arg construction).
-- The functions are SELF-recursive only — no cycle into the SCC itself
-- once the callbacks are abstracted. CompTheorem.agda binds aliases
-- (composeCompFits = composeCompFitsExt substDerivTmCompCF) inside its
-- mutual block to keep call-site syntax unchanged.

composeCompFitsExt
  : (substRec : SubRecTy)
  -> {n : ℕ} -> {gamma delta : Ctx} {rho sigma : Subst} {t : RawTerm} {T : RawType}
  -> (outer : FitsSubst [] gamma rho)
  -> ComputableFits n outer
  -> (inner : FitsSubst gamma delta sigma)
  -> (dt : Derivable (hasTy gamma t T))
  -> Acc LexLt (substTaskLexMeasure dt)
  -> ComputableFits n (composeFits outer inner)
composeCompFitsExt substRec {n} outer cOuter (fitsNil wf) dt accDt = compFitsNil
composeCompFitsExt substRec {n} {rho = rho} outer cOuter
  ...
  substCompFits
    ...
    (compFitsCons
      ...
      (composeCompFitsExt substRec outer cOuter inner dt accDt)
      (subst
        (λ T -> Computable n (hasTy [] (subTm rho t) T))
        (subTyComp rho sigmaTail A)
        (substRec dtInner outer cOuter (LexLt-wf _))))

composeCompEqFitsExt
  : (eqSubRec : EqSubRecTy)
  -> (fitsEqToCompFitsEqCb : FitsEqToCompFitsEqTy)
  -> {n : ℕ} -> {gamma delta : Ctx} {rho eta sigma : Subst} {t : RawTerm} {T : RawType}
...
composeCompEqFitsExt eqSubRec _ {n} outer cOuter (fitsNil wf) dt accDt = compFitsEqNil
composeCompEqFitsExt eqSubRec fitsEqToCompFitsEqCb {n} {rho = rho} {eta = eta} outer cOuter
  (fitsCons {sigma = sigmaTail} {A = A} {t = t} inner dtInner) dt accDt =
  substCompFitsEqLeft
    (sym (compSubCons rho t sigmaTail))
    (substCompFitsEqRight
      ...
      (compFitsEqCons
        ...
        (composeCompEqFitsExt eqSubRec fitsEqToCompFitsEqCb outer cOuter inner dt accDt)
        (subst
          (λ T -> Computable n (termEq [] (subTm rho t) (subTm eta t) T))
          (subTyComp rho sigmaTail A)
          (eqSubRec dtInner outer (fitsEqToCompFitsEqCb outer) (LexLt-wf _)))))

composeCompFitsEqExt
  : (subEqRec : SubEqRecTy)
  -> (fitsToCompFitsCb : FitsToCompFitsTy)
  -> {n : ℕ} -> {gamma delta : Ctx} {rho sigma tau : Subst} {t u : RawTerm} {T : RawType}
  -> (outer : FitsSubst [] gamma rho)
  -> ComputableFits n outer
...
composeCompFitsEqExt subEqRec _ {n} outer cOuter (fitsEqNil wf) dtu accDtu = compFitsEqNil
composeCompFitsEqExt subEqRec fitsToCompFitsCb {n} {rho = rho} outer cOuter
  ...
  substCompFitsEqLeft
    ...
    (substCompFitsEqRight
      ...
      (compFitsEqCons
        ...
        (composeCompFitsEqExt subEqRec fitsToCompFitsCb outer cOuter inner dtu accDtu)
        (subst
          (λ T -> Computable n (termEq [] (subTm rho t) (subTm rho u) T))
          (subTyComp rho sigmaTail A)
          (subEqRec dtuInner outer (fitsToCompFitsCb outer) (LexLt-wf _)))))

composeCompEqFitsEqExt
  : (eqSubEqRec : EqSubEqRecTy)
  -> (fitsEqToCompFitsEqCb : FitsEqToCompFitsEqTy)
  -> {n : ℕ} -> {gamma delta : Ctx} {rho eta sigma tau : Subst} {t u : RawTerm} {T : RawType}
  -> (outer : FitsEqSubst [] gamma rho eta)
  -> ComputableFitsEq n outer
  -> (inner : FitsEqSubst gamma delta sigma tau)
  -> (dtu : Derivable (termEq gamma t u T))
...
composeCompEqFitsEqExt eqSubEqRec _ {n} outer cOuter (fitsEqNil wf) dtu accDtu = compFitsEqNil
composeCompEqFitsEqExt eqSubEqRec fitsEqToCompFitsEqCb {n} {rho = rho} {eta = eta} outer cOuter
  (fitsEqCons {sigma = sigmaTail} {tau = tauTail} {A = A} {t = t} {u = u} inner dtuInner) dtu accDtu =
  substCompFitsEqLeft
    (sym (compSubCons rho t sigmaTail))
    (substCompFitsEqRight
      ...
      (compFitsEqCons
        ...
        (composeCompEqFitsEqExt eqSubEqRec fitsEqToCompFitsEqCb outer cOuter inner dtu accDtu)
        (subst
          (λ T -> Computable n (termEq [] (subTm rho t) (subTm eta u) T))
          (subTyComp rho sigmaTail A)
          (eqSubEqRec dtuInner outer (fitsEqToCompFitsEqCb outer) (LexLt-wf _)))))
